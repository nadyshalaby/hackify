...
# [79] THE STANDING-MEMBER INVARIANT, split out of 77-reviewer-roster.sh in the
# sprint that file reached 499 of 500 LOC and could not take another line.
#
...
# 'A, D and F are gated on evidence'.
#
...
yellow "[79] the standing-member-is-B invariant, over every file that claims one"
...
for rr_d in "${RR_SM_DIRS[@]}"; do
  [ -d "$rr_d" ] && continue
  red "  FAIL [79] standing-member scan root '$rr_d' is not a directory, so every roster claim under it has silently left coverage"
  FAILED=$((FAILED + 1))
  RR_SM_ROOTBAD=1
done
if [ "$RR_SM_ROOTBAD" -eq 0 ]; then
  green "  ok   all ${#RR_SM_DIRS[@]} [79] standing-member scan directory roots resolve"
fi

RR_SM_FILEBAD=0
for rr_d in "${RR_SM_FILES[@]}"; do
  [ -s "$rr_d" ] && continue
  red "  FAIL [79] standing-member scan file root '$rr_d' is missing or empty, so the roster claim it carries has silently left coverage"
  FAILED=$((FAILED + 1))
  RR_SM_FILEBAD=1
done
if [ "$RR_SM_FILEBAD" -eq 0 ]; then
  green "  ok   the [79] standing-member scan file root(s) ${RR_SM_FILES[*]} exist and are non-empty"
fi
...
while IFS= read -r -d '' rr_f; do
  ...
  ' "$rr_f" 2>/dev/null); then
    red "  FAIL the [79] standing-member scan discovered a file it could not then read, so its roster claim was never judged: $rr_f"
    FAILED=$((FAILED + 1))
    continue
  fi
...
if [ "$RR_SM_HITANCHOR" -eq 1 ]; then
  green "  ok   the [79] standing-member scan reaches $RR_SM_ANCHOR, so its discovery still resolves"
else
  red "  FAIL the [79] standing-member scan found no 'standing member' claim in $RR_SM_ANCHOR, so its discovery is broken and the scan below measures nothing"
  FAILED=$((FAILED + 1))
fi
...
if [ "$RR_SM_RC" -gt 1 ]; then
  red "  FAIL the [79] standing-member discovery exited $RR_SM_RC, so a root under ${RR_SM_DIRS[*]} ${RR_SM_FILES[*]} was unreadable and every roster claim inside it went unjudged"
  FAILED=$((FAILED + 1))
fi
...
if [ "$RR_SM_N" -eq 0 ]; then
  ...
  red "  FAIL the [79] standing-member scan matched no file at all under ${RR_SM_DIRS[*]} ${RR_SM_FILES[*]}, so its clean verdict measured nothing"
  FAILED=$((FAILED + 1))
