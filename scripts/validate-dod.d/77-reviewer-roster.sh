...
# spec-reviewer counts, dispatch counts, and the adjudicator's reviewer-report
...
#   wider: the loop below runs 60 tokens per file against [70]'s 23, and every
#   shared range form is BROADER here. '5-to-6' matches '5-to-6-reviewer',
#   '5-to-6 panel' and '5-to-6 spec reviewers' alike, where [70]'s two fixed
#   phrases match only their own wording. The reverse never occurs, no [70]
#   token subsumes one of the 60. The six adjudicator bans at the bottom of
#   this block are scoped to one file and counted separately from those 60.
#
...
#   which today is 12 files: 7 under skills/ and 5 under agents/. rules/ and
#   commands/ are scanned too and currently match nothing; they are in the root
#   list because a roster claim can land there, not because one has. A hand-kept
#   list would have been worse than useless here: BOTH roster defects lived in
#   files the six-file list does not name, so a list-scoped version of this
#   check would have run green over the two sentences it exists to catch. The
#   list is not the unit of coverage, the claim is.
#
...
# one rule here that could redden prose an author believes is fine (something
# like "the panel has exactly one standing member"). No such sentence exists
# today. It is bought anyway because subject-free is the exact shape of the
# f-coherence defect: a "**Standing member.**" heading whose body then names the
# wrong reviewer. The fix an author owes is one word, name the letter.
#
...
#
# THE ADJUDICATOR'S REPORT INPUT is pinned at the bottom of this block. It is
# the same defect one layer down: a roster hardcoded as INPUT slots rather than
# as a sentence. review-and-verify.md's escalation prompt enumerated
# {{reviewer_a_report}}, {{reviewer_b_report}} and {{reviewer_d_report}}, so the
# adjudicator structurally could not read a finding from E or F, the two the
# gate adds most often. It was fixed by collapsing all three into one
...
yellow "[77] reviewer-roster drift: count grammar over 6 files (2 no other check reaches, a wider token set on the 4 shared with [70]) plus the adjudicator's report input, and the standing-member-is-B invariant over every file that claims one"
...
for f in $RR_FILES; do
  ...
  for t in '2 reviewers in parallel' '3 reviewers in parallel' '4 reviewers in parallel' '5 reviewers in parallel' '6 reviewers in parallel'; do
    check_no_token "$t" "$f"
  done
  for t in 'two reviewers in parallel' 'three reviewers in parallel' 'four reviewers in parallel' 'five reviewers in parallel' 'six reviewers in parallel'; do
    check_no_token "$t" "$f"
  done
...
# The escalation adjudicator takes ONE count-agnostic report input, never a slot
...
if [ -s "$RR_RAV" ]; then
  ...
  for t in reviewer_a_report reviewer_b_report reviewer_c_report reviewer_d_report reviewer_e_report reviewer_f_report; do
    check_no_token "$t" "$RR_RAV"
  done
fi
...
# Vacuous-pass guard, part one: the roots. grep -r over a missing directory
# writes to stderr, which is suppressed here, and contributes no files. So
# renaming a tree would take every roster claim under it out of coverage while
# the scan below stayed green. All four roots are asserted, not just the one the
# anchor lives in, because agents/ alone carries 5 of the 12 covered files.
RR_SM_ROOTBAD=0
for rr_d in skills agents rules commands; do
  [ -d "$rr_d" ] && continue
...
if [ "$RR_SM_ROOTBAD" -eq 0 ]; then
  green "  ok   all 4 [77] standing-member scan roots resolve"
fi

RR_SM_SET=$(/usr/bin/grep -rlIi -- 'standing member' skills agents rules commands 2>/dev/null)
RR_SM_N=$(printf '%s\n' "$RR_SM_SET" | /usr/bin/grep -c .)
...
# known to carry the phrase. This asserts the MECHANISM, not a file set. Fed by
# here-string rather than a pipe: grep -q stops at the first match, and a piped
# writer killed by SIGPIPE would surface through pipefail as a false failure.
if /usr/bin/grep -qxF -- "$RR_SM_ANCHOR" <<<"$RR_SM_SET"; then
...
# Every occurrence on every discovered line, subject located rather than matched.
RR_SM_BAD=$(printf '%s\n' "$RR_SM_SET" | while IFS= read -r rr_f; do
  [ -n "$rr_f" ] || continue
  awk '
    {
...
if [ "$RR_SM_N" -eq 0 ]; then
  ...
  red "  FAIL the [77] standing-member scan matched no file at all under skills/ agents/ rules/ commands/, so its clean verdict measured nothing"
  FAILED=$((FAILED + 1))
elif [ -z "$RR_SM_BAD" ]; then
  green "  ok   every 'standing member' claim names B ($RR_SM_N files scanned under skills/ agents/ rules/ commands/)"
else
