---
name: skillsmith
description: Authors NEW skills that conform to hackify's binding contracts. NOT a generic Claude Code skill creator. Produces SKILL.md files under skills/<slug>/ that pass every check the validate-dod.sh harness runs against hackify-conformant skills, the 8-section sub-agent contract, the 4-section Wizard contract, the frontmatter schema, the name regex, the mandatory OUTPUT word-cap on every embedded sub-agent prompt, and Haiku-portability via zero soft-language tolerance in the body. Auto-discovery triggers, invoke when the user says `/hackify:skillsmith`, `author a hackify skill`, `create a new skill for hackify`, `make a hackify-style skill`, or `new hackify skill`. Self-validates every authored skill before declaring done. Explicit non-goal, does NOT author arbitrary Claude Code skills, does NOT replace the skill-creator plugin, and does NOT write ordinary scripts or programs.
---
...
4. **Sub-agent prompts conform to the 8-section contract, and their REQUIRED READING actually binds.** Every embedded sub-agent prompt inside the skill body has all 8 sections. ROLE, INPUTS, REQUIRED READING, OBJECTIVE, METHOD, VERIFICATION, SEVERITY (review/audit templates only, omitted entirely from build/research templates), OUTPUT. Each section header appears on its own line in the form `**SECTION-NAME**.`, the bold marker plus a literal trailing period, matching the canonical templates in `skills/hackify/references/parallel-agents/template-contract.md`.

   **Counting the sections is HALF of this check.** A prompt carries all 8 headers and still dispatches an agent that opens nothing, which is the exact defect this half exists to catch: naming a section named REQUIRED READING is not the law, naming the files that bind the agent is. Nothing else injects hackify's rules into a sub-agent, the always-on hook fires on a USER prompt and a dispatch is not one, so a rule file reaches an authored agent only because its REQUIRED READING section names it. All five bars below are met, each COPIED from the skeleton in `skills/hackify/references/parallel-agents/template-contract.md` rather than restated, because a restatement that drifts is the failure this bar was written for:

   - **`{{plugin_root}}` is a declared INPUT.** Every REQUIRED READING path is anchored to it, so the prompt's INPUTS section declares it the way it declares every other input. A list anchored to an input the prompt never declares resolves to nothing at dispatch time.
   - **The three fixed paragraphs are carried verbatim.** The opening line, in whichever wording `skills/hackify/references/parallel-agents/template-contract.md` sanctions for the list this prompt actually carries, since a gated list and a conditional list each reword that one sentence and nothing else, plus its sentence putting each absolute path on `{{plugin_root}}`; the paragraph beginning "This list is EXHAUSTIVE and CLOSED."; and the missing-canon paragraph, which stops the agent before METHOD step 1, reports `missing canon: <path>`, and produces no other output.
   - **Every listed path is anchored to `{{plugin_root}}`,** one numbered entry per file, each carrying one clause naming what that file governs for THIS role rather than what the file is about in general. A bare or repo-relative path fails this check.
   - **The dangling-citation rule holds.** Inside a template's FENCED BLOCK, a plugin file may be CITED only if that same file also appears on that template's REQUIRED READING list. The repair is one of two things, never a third: bind the file or drop the citation. Prose outside the fenced block is addressed to the parent and cites freely.
   - **The mandatory VERIFICATION item is present, in the ONE wording this prompt's list earns.** `skills/hackify/references/parallel-agents/template-contract.md` makes a yes/no item naming REQUIRED READING mandatory in EVERY template, and defines THREE wordings of it, exactly one of which appears, never two and never none. A list of plain entries only takes "Did you open every REQUIRED READING path in full before METHOD step 1? (yes / no)". A list carrying any step-bound entry takes "Did every REQUIRED READING path resolve before METHOD step 1, and did you open every entry in full at the step its own entry names? (yes / no)". A list carrying any CONDITIONAL entry takes "Did every REQUIRED READING path resolve before METHOD step 1, and did you open in full, before METHOD step 1, every entry whose condition your dispatch met? (yes / no)". The item sits on ONE unwrapped physical line, so a reader and a matcher see the same item. A prompt whose line does not match its own list fails this bar in one of two directions, both defects: the universal line over a conditional list demands a read the prompt forbids, and a variant over a list earning neither widens the bar into one it passes by construction. A list mixing step-bound and CONDITIONAL entries is out of contract, and the repair is to pick one shape.

   A prompt whose METHOD is explicitly GATED binds an individual entry at a NAMED step instead of step 1, under the conditions `skills/hackify/references/parallel-agents/template-contract.md` states and no others. LONG IS NOT GATED: a prompt that merely does not need a file until step 7 binds it at step 1 like every other prompt.
...
- **Does NOT author generic Claude Code skills.** The output is hackify-conformant only, frontmatter schema, 8-section sub-agent contract, 4-section Wizard contract, mandatory body sections, and soft-language ban. For generic skill authoring, use the `skill-creator` plugin (separate, not bundled with hackify).
...
| "The skill is small, the 8-section contract is overkill" | The 8-section contract applies to every embedded sub-agent prompt regardless of skill size, 8 sections, with SEVERITY required only for review/audit-role prompts (build/research prompts omit it entirely). A small skill with one sub-agent dispatch still owes that dispatch its full section set. |
...
skills/hackify/references/parallel-agents/template-contract.md   <- 8-section sub-agent contract (binding)
