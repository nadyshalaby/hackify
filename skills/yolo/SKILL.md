...
| **3. Implement** | Parallel implementation waves with per-task file allowlists, dispatched **by registered agent type** (`hackify:wave-task-implementer`) with only its INPUTS, the shared `{{repo_brief}}` among them, never by pasting a template. Same as full hackify Phase 3, including BOTH wave-end scouts over the wave-touched files, perf-scout (`../hackify/references/perf-scout.md`) and law-scout (`../hackify/references/law-scout.md`), trivial in-allowlist candidates fixed in-wave, everything else staged for Phase 5. | Wave discipline is what makes parallel safe. |
