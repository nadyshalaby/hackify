...
Phase 1  (clarify + exploration + wizard if ambiguous + in-chat goal anchor)
  ...
  → Phase 5  (both scouts re-run, then the evidence-gated panel: B/C/F standing,
              A/D gated on surface, E on UI-bearing; folded lenses handed to B;
...
| **3. Implement** | Parallel implementation waves with per-task file allowlists, dispatched **by registered agent type** (`hackify:wave-task-implementer`) with only its INPUTS, one agent per task BATCH (same-module tasks grouped, capped at 3, a task with no module sibling goes alone), the shared `{{repo_brief}}` among them, never by pasting a template. Same as full hackify Phase 3, including BOTH wave-end scouts over the wave-touched files, perf-scout (`../hackify/references/perf-scout.md`) and law-scout (`../hackify/references/law-scout.md`), trivial in-allowlist candidates fixed in-wave, everything else staged for Phase 5. | Wave discipline is what makes parallel safe. |
