...
| **3. Implement** | Implementation waves in dependency order with per-task file allowlists, dispatched **by registered agent type** (`hackify:wave-implementer`) with only its INPUTS, ONE agent for the whole wave however wide it is (no cap, no module split), carrying the shared `{{repo_brief}}`, never by pasting a template. Same as full hackify Phase 3, including BOTH wave-end scouts over the wave-touched files, perf-scout (`../hackify/references/perf-scout.md`) and law-scout (`../hackify/references/law-scout.md`), trivial in-allowlist candidates fixed in-wave, everything else staged for Phase 5. | Wave discipline is what keeps the order safe, and one agent per wave keeps its context in one place. |
