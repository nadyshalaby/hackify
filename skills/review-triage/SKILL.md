...
**Path A. Phase 5 internal.** Triggered automatically inside the full hackify workflow once the Phase 5 parallel-reviewer dispatch returns. The parent has just received one report per lens the Phase 5 evidence gate put on the panel. Reviewer B stands on every wave and covers quality, layering, engineering law, plan consistency and scope. Security & correctness, performance and cross-module coherence report in when the gate ran them, and design conformance reports in on a UI-bearing diff. Before the parent decides what to fix, what to push back on, and what to defer to the Retrospective, this skill produces the structured per-finding response. The Critical / Important / Minor severity tags from the reviewers are mirrored into the table's Severity column verbatim.
