---
name: hackify
description: One unified end-to-end dev workflow for ANY substantive task, feature, bug fix, refactor, redesign, design, debug, migration, or research-then-build. Driven by one markdown work-doc at <project>/docs/work/, with every clarifying question asked up front in one batched questionnaire, a hard gate before code is written, contention-first implementation (a solo foundation wave, then concurrent module tracks, then a solo assembly wave that boots the system) and parallel review waves, mandatory evidence before claims, an always-on ship bar that proves the app builds and boots and serves the touched flow, and file-driven pause/resume across sessions. The default route for any substantive prompt, auto-fires on broad-spectrum verbs (add, build, implement, refactor, redesign, restyle, migrate, debug, polish, audit) AND on architecture/scope/security surface (auth, crypto, migration, secret, token, password, schema, data model, API surface, refactor everywhere, across all). The retired autopilot phrases route here too and now get the full gated workflow rather than an autopilot, so "yolo", "just do it", "don't ask me", "no questions", "fully autonomous", "auto mode" and "go full auto" all land on this skill with its plan gate and its work-doc intact. Invoke even when the user does not say "use the workflow", carve-outs are trivial factual Q&A, single-line typo fixes, and pure read-only inspection. When in doubt, invoke this skill, escalation to full ceremony is free, demotion is not.
---
...
| 3 Implement | Foundation wave (solo, every contended write) → concurrent module tracks, each to DONE → assembly wave (solo, mount + reconcile + real boot). Each wave goes to ONE foreground agent |
...
**Parallelism is the default.** Whenever 2+ pieces of work are independent (clarify research, code review concerns, cross-package verification) dispatch foreground subagents in one message. Phase 3 gives it up only where it buys something: when a wave's tasks have a shared read surface, reading the same types, the same neighbouring code and the same conventions, ONE agent takes the whole wave. A wave whose tasks share no read surface MAY go out as concurrent waves, one agent each, but only when all three conditions of the partition test in [references/contention-dispatch.md](references/contention-dispatch.md) hold. Waves still run in dependency order and same-file tasks still split across waves. Spec review is dispatched to a subagent too, it is just a single reviewer rather than a fan-out.
...
Whenever 2+ pieces of work are independent, **dispatch foreground subagents in parallel in a single message**. Never sequential when independent. Phase 3 scopes it rather than dropping it: a planned wave goes to ONE agent whatever its width when its tasks share a read surface, the waves the partition test clears MAY go out together in one round, once they have been intersected pairwise, and the wave log records which task IDs each agent carried.
...
| 3 | Implementation waves, one subagent per wave carrying every task in it, always `hackify:implementer`, with `{{sibling_tracks}}` set to `none` on a solo foundation, assembly or single-track wave and naming the siblings on concurrent tracks (both scouts run twice: each agent over its own allowlist before it returns, the parent over what that round's waves declared at round-end) | MANDATORY |
...
| "This wave only has one task, so there's nothing to parallelize" | Not inside it: a wave is ONE agent whatever its width, and a one-task wave is the same dispatch with one task in it, never a licence to write it yourself. Whether it MAY run beside another wave is the partition test's question, and a passing test PERMITS that rather than ordering it; the task count answers nothing either way. |
