...
- **The coherence lens is never silently absent** (Reviewer F). A wave's implementer is blind to the waves that ran before it and to every line of pre-existing code, which is exactly where a producer and its consumers drift apart; F is the only lens that checks producer against consumer. It runs on every non-trivial diff, with no gate left to fold it away (`references/parallel-agents/phase-5-multi-review-f-coherence.md`).
- **Refute before you fix, and review once.** Findings are judged by an adversarial refuter before a fix is spent on them, and Phase 5 runs exactly one panel and one refuter, then ends when the survivors are fixed.
...
**Parallelism is the default.** Whenever 2+ pieces of work are independent (clarify research, code review concerns, cross-package verification) dispatch foreground subagents in one message. Phase 3 gives it up only where it buys something: when a wave's tasks have a shared read surface, reading the same types, the same neighbouring code and the same conventions, ONE agent takes the whole wave, packed up to the per-agent task budget. A wave whose tasks share no read surface MAY go out as concurrent waves, one agent each and up to the concurrent-wave budget, but only when all three conditions of the partition test in [references/contention-dispatch.md](references/contention-dispatch.md) hold. Both budgets are stated as digits in that one file and named rather than numbered everywhere else, this line included. Waves still run in dependency order and same-file tasks still split across waves. Spec review is dispatched to a subagent too, it is just a single reviewer rather than a fan-out.
...
- **One item `in_progress` at a time, no jumping ahead.** A later phase cannot start until the current phase's exit artifact exists and its item is `completed`. No phase is skipped: a carve-out is marked `completed` with a one-line reason, never deleted. Parallelism lives *inside* a phase (the Phase 5 reviewer panel, Phase 1 research agents), never across phases. **Concurrent Phase 3 waves are inside a phase, so this law never forbade them:** waves the partition test clears run at the same time within Phase 3, and Phase 3 still closes before Phase 4 opens.
...
**One panel, one refuter, then the phase ends.** **Phase 5 dispatches exactly ONE reviewer panel and ONE refuter, and the phase ends when the surviving findings are fixed.** The fix waves address every finding that survived refutation, the verify triad runs on the touched scope, and Phase 5 is over: no second panel and no re-scan, however much the fixes changed. A defect a fix introduces is fixed in the same fix sequence and reported; anything still unresolved when the fixes are done goes to you as a written list, never as another round. A finding nobody could restate does not open a round either: the claim gets reworded inside the round that is already running, the reworded finding rejoins it, and one that still cannot be worded lands in that same written list. **The reviewed diff is `git diff <base>..HEAD -- . ':(exclude)docs/work/*'`**, because the work-doc is the ruler the diff is measured against and cannot also be the measured. The cap trades away one real guarantee, that the panel saw the post-fix diff, and [references/phases/phase-5-review.md](references/phases/phase-5-review.md) states the trade and the evidence behind it rather than pretending the risk went away.
