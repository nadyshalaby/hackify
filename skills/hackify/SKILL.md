...
| 3 | Implementation waves, one subagent per wave carrying every task in it (both scouts run twice: each agent over its own allowlist before it returns, the parent over what that round's waves declared at round-end) | MANDATORY |
...
| "These two waves look independent, I'll just run them together" | Only once the partition test passes on all three of its conditions ([references/phases/phase-3-implement.md](references/phases/phase-3-implement.md)). A shared file, an import edge in either direction, or one serial resource held by both, and ONE agent takes the whole thing. |
