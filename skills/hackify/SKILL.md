...
**Agent selection per wave.** Foundation and assembly go to `hackify:wave-implementer`, and so does a single-track round, because with no siblings running beside it the blind-sibling rules protect nothing and only cost context. Two or more concurrent tracks go to `hackify:module-implementer`, which carries its own database, an explicit account of the siblings it cannot see, and the rule that it builds against planned contracts rather than landed code.

Dispatch each wave by agent type, passing only its INPUTS. Do not open the template to paste it (`references/parallel-agents/README.md`). Exactly ONE subagent takes the whole wave however wide it is when its tasks share a read surface: there is no width valve and no split by module hunch, because the partition test in [references/contention-dispatch.md](references/contention-dispatch.md) is the only thing that may split a wave, and the waves it clears go into one round only after you have intersected them pairwise, one agent each. Each task still sits under its own strict file allowlist and the wave is bounded by their union, which never widens what one task may touch. **A track that finds a defect in shared code reports it, it does not fix it**, because reaching outside the allowlist falsifies the partition every sibling was dispatched under, retroactively. Read the wave plan out of the Phase 2.5 report rather than rebuilding it, and note what that does not ban: the parent MAY merge consecutive waves when no file collides inside the merged set and no dependency edge crosses the merge, because "read the plan rather than rebuild it" bans re-planning, not merging. **Two of those INPUTS make the agent REFUSE when they reach it unfilled, so settle both before you dispatch.** `{{repo_brief}}` is the `### Repo Brief` block you built at the end of Phase 2 (in the work-doc, [references/repo-brief.md](references/repo-brief.md)), passed verbatim so each wave's agent stops rediscovering the same stack, test command and layering rules the wave before it already derived; if the block is empty, fill it now rather than dispatching over the placeholder. `{{exclusive_resources}}` carries every exclusive resource THAT wave holds, one per line, or the literal `none`, and never an empty value, because an empty one says the dispatcher never decided. **Those two are `hackify:wave-implementer`'s pair. `hackify:module-implementer` takes neither** and refuses on any of its fifteen arriving unfilled, of which `{{sharp_invariants}}` and `{{database_name}}` are the two whose absence is silent rather than loud: a blind track steps on an unnamed trap reliably, and a track with no database of its own runs its integration suite against the shared one a sibling is truncating. Its full input list, and where each value comes from, is the `### Module briefs` block the work-doc carries for a concurrent round ([references/work-doc-template.md](references/work-doc-template.md)).
...
| 3 | Implementation waves, one subagent per wave carrying every task in it, foundation and assembly on `hackify:wave-implementer` and concurrent tracks on `hackify:module-implementer` (both scouts run twice: each agent over its own allowlist before it returns, the parent over what that round's waves declared at round-end) | MANDATORY |
