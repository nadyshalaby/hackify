...
**Exit only on a settled diff.** The loop may only end on a FULL round, the full gated panel over the full range, that finds nothing AND leaves every byte of the reviewed diff covered by a live verdict, one recorded against the blob hash the file still carries. **The reviewed diff is `git diff <base>..HEAD -- . ':(exclude)docs/work/*'`**, because the work-doc is the ruler the diff is measured against and cannot also be the measured; without the exclusion, writing a round's result into the work-doc kills the work-doc's own verdict and mandates another round, and the loop cannot close. Every lens on that round is dispatched with a `settle `-prefixed scope. Middle rounds are scoped to the fix diff and can never close the loop, no matter how clean.
