...
### Default: one merged all-lens reviewer + self-review

For any non-trivial diff (anything beyond a one-line typo / config-only change), Phase 5 dispatches ONE merged all-lens reviewer (`hackify:reviewer`) over the diff, carrying security and correctness, performance, design conformance, cross-module coherence, and quality with layering and plan consistency, as five gated passes. **The panel is not the automatic route any more, and it is one request away:** ask for the reviewer panel on a diff and Phase 5 dispatches it as foreground reviewers in parallel in a single message instead, roster unchanged. On such a round, A, B, D and F each run on every non-trivial diff, and E joins on a UI-bearing one. B carries quality/layering **and plan consistency** over one read. A is security/correctness, D is performance, F is cross-module coherence. E design-conformance is the one conditional lens, and it is omitted rather than folded when the diff has no UI surface. Cap at 5. The panel table and the reason the evidence gate was retired are in [phases/phase-5-review.md](phases/phase-5-review.md). (Reviewer C folded into B in v0.13.0: both ran on every wave, so a permanent merge took a saving nothing else could reach.)
...
Dispatch templates: `parallel-agents/phase-5-multi-review-merged.md` for the default route, then, for a panel round, `parallel-agents/phase-5-multi-review-a-security.md`, `phase-5-multi-review-b-quality-plan.md`, `phase-5-multi-review-d-performance.md`, `phase-5-multi-review-e-design.md`, `phase-5-multi-review-f-coherence.md`. Any other distinct concern takes a specialist from `phase-5-escalation.md` instead of E.
