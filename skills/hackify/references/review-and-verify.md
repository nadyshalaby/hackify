...
2. **Refute before you fix.** Dispatch the adversarial refuters ([parallel-agents/phase-5-refute.md](parallel-agents/phase-5-refute.md)) over the table in one message: one refuter per **Critical** on the reproduction lens, plus one batched refuter for the whole Important+Minor set, and a SECOND refuter on the authority lens only for those Criticals whose first refuter came back REFUTED. Their verdicts fill the `Decision` and `Evidence` columns. **The default is to keep the finding**, uncertainty is never a refutation, and a Critical dies only when BOTH of its refuters refute it with a file:line counter-citation. That last rule is exactly why the second refuter is conditional: once the first upholds, the finding survives and a second verdict cannot change it, so the surviving set is identical either way. This is how a `push-back` earns its required evidence; a Critical may never be `push-back` on a single refutation, escalate to the adjudication reviewer instead.
...
4. **Re-scan to prove zero, on a settled diff.** After each batch, re-run the verify triad on the touched scope, re-run both scouts, and re-dispatch the reviewers over the new diff, carrying each one's inputs again (B's `{{folded_lenses}}` included, unchanged from round one). **A round that changed any code mandates another round**, its clean result describes the pre-fix diff, not the current one. Exit only when a full round produces zero new findings AND every byte of the reviewed diff, `git diff <base>..HEAD -- . ':(exclude)docs/work/*'`, is covered by a live verdict, one recorded against the blob hash the file still carries. The exclusion is not optional and not a shortcut: the work-doc is the ruler the diff is measured against and cannot also be the measured, so grading it makes every round invalidate its own verdict by recording its result. That final round is FULL only when every dispatched lens that takes a scope echoed a `settle `-prefixed scope, F echoed `settle all`, and B echoed `Round: settle`; F never carries a verdict over, because coherence can break without either file changing, and a lens that received a bare pathspec list was running a middle round and cannot close the loop. That is the difference between "the last fixes were never reviewed" and "the work is clean".
