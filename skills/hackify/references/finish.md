...
**Step F emits no second document.** What the user takes away is the work-doc itself, published as a page and kept current since Phase 2 step 1, so the update log you just printed reaches them on the link they already hold, beside the Evidence Ledger, the findings and the ledger block that same doc already carries. Nothing is rendered, and nothing is written by hand. Do not rebuild a renderer, a template or a payload to stand in for one: [work-doc-artifact.md](work-doc-artifact.md), "What gets published".

**And where the runtime can publish a page, the final republish rides Step D's closing edit.** A path is something the user has to open by hand and cannot send to anyone; a link is the thing they asked for, and they have had this one since Phase 2. Republishing the same file keeps the same URL, so this last one just leaves that link showing the finished sprint. Say it out loud again only if it changed or the user asks: a link that never moves is the proof the design worked, and re-printing it at every boundary is noise. On a runtime with no publish tool, say so in one line, name the path, and carry on. **Publishing is never a precondition for closing the ledger.** 6d's exit artifact is the printed log plus the work-doc on disk, never a link, so a sprint finishes on a runtime that cannot publish anything. The page contract: [work-doc-artifact.md](work-doc-artifact.md). Per-runtime support and the degrade path: [runtime-adapters.md](runtime-adapters.md), "Native-tier enhancements".
