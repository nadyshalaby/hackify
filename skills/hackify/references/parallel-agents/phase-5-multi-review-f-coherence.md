...
- **On the panel unconditionally once the panel is what runs, and never gated on size or on a visible seam.** Whether the panel runs at all is a routing question settled above and elsewhere: the round's default reviewer is the five-agent panel, and the merged all-lens `hackify:reviewer` is the named, lower-cost opt-out a user reaches by asking for it directly. Once the panel IS the round's reviewer, F is on it unconditionally and dispatches in the same single parent message as the rest of it. F carried a SECOND gate on top of that, on the diff crossing a module boundary; THAT gate is retired, and F now runs whether or not you can see a seam, and whatever the size of the diff. Most non-trivial waves cross a seam anyway, because a wave's implementer is blind to the waves that ran before it and to every line of pre-existing code, and F is the only lens that compares producer against consumer. A diff you were sure stayed inside one module is exactly the diff whose seam nobody looked for. The carve-out is the same as the rest of the panel: a purely one-line typo / comment / config-only diff.
