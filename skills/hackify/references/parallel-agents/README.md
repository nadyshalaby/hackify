...
| `hackify:reviewer-security` | 5 A, security & correctness. A panel lens: on request, or when the panel is being measured against the merged reviewer, never automatically | `project_root`, `base_sha`, `head_sha`, `work_doc_path`, `repo_brief`, `review_scope` | `phase-5-multi-review-a-security.md` |
| `hackify:reviewer-quality-plan` | 5 B, quality, layering, engineering law **and** plan consistency, scope & goal drift, closing with a mandatory completeness section that asks what the review did not reach and files the answer as findings. Never sliced. A panel lens: on request, or when the panel is being measured against the merged reviewer, never automatically | `project_root`, `base_sha`, `head_sha`, `work_doc_path`, `repo_brief`, `project_rules_path`, `changelog_path`, `law_scout_report`, `task_file_index`, `metrics_table` | `phase-5-multi-review-b-quality-plan.md` |
| `hackify:reviewer-performance` | 5 D, performance. A panel lens: on request, or when the panel is being measured against the merged reviewer, never automatically | + `perf_scout_report`, `review_scope` | `phase-5-multi-review-d-performance.md` |
| `hackify:reviewer-design` | 5 E, design conformance (UI-bearing only). A panel lens: on request, or when the panel is being measured against the merged reviewer, never automatically | + `design_spec_path`, `reference_images` | `phase-5-multi-review-e-design.md` |
| `hackify:reviewer-coherence` | 5 F, cross-module coherence. A panel lens: on request, or when the panel is being measured against the merged reviewer, never automatically | + `task_file_index` | `phase-5-multi-review-f-coherence.md` |
...
| `hackify:reviewer` | 5, THE DEFAULT REVIEWER IN EVERY MODE, the merged all-lens reviewer: one agent carrying A, D, E, F and B over one read of the diff as five gated passes, each opening its own canonical lens file at the start of its own pass and emitting that lens's complete report before the next pass reads anything, then a widened completeness section over all five passes and a roll-up counted off the artifacts. **Full mode routes here too**, and the panel in [`../phases/phase-5-review.md`](../phases/phase-5-review.md) becomes the route somebody asks for by name rather than the one a round takes on its own. **The measurement is part of the decision, not an argument against it**, so quote it when you route a round: head to head on one real diff this shape returned 16 findings against the panel's 29, and 1 Critical against 4. Quick was already here, because quick always ran ONE reviewer carrying every lens, so what changed there was a gated prompt replacing an ad-hoc one | `project_root`, `base_sha`, `head_sha`, `work_doc_path`, `project_rules_path`, `changelog_path`, `law_scout_report`, `perf_scout_report`, `task_file_index`, `metrics_table`, `design_spec_path`, `reference_images`, `repo_brief`, `plugin_refs_dir`. All 14 are required and none takes `none` except `design_spec_path` and `reference_images`; there is no `review_scope`, because the agent carries B and B is never sliced | `phase-5-multi-review-merged.md` |

### The panel rows stay, and here is what they are for now

Rows A, B, D, E and F are registered, dispatchable and unchanged. What moved is the ROUTE: a review round no longer reaches for them on its own. Three jobs keep every one of them in this table.

- **On request.** Somebody asks for the panel on a specific diff, and it goes out in one parent message exactly as before, all its lenses at once.
- **Escalation.** The merged reviewer's report reads thin on a surface this diff is loaded with, so you dispatch that one lens again by type rather than re-running everything.
- **Measurement.** The pass bar the next sprint sets for the merged reviewer is measured against what these lenses find on the same diff. Delete the rows and that bar stops being provable, which is why nothing here was deleted.

Head to head on one real diff, the merged reviewer returned 16 findings and 1 Critical where the panel returned 29 and 4. The panel finds more. The default moved anyway, and that is the trade the decision makes rather than a fact it hides, so say it out loud when you route a round.
...
- **Retired in v0.13.0,** taking the plugin from 12 agent types to 9. The `module-implementer` type then landed, making 10, and 0.17.1 merged it with `wave-implementer` into the single `hackify:implementer`, which took it back to 9. `hackify:reviewer` then landed, for quick mode first and now as the default reviewer in every mode, so the table above lists 10 and `agents/` holds 10 definitions. **Those two retired names carry no `hackify:` prefix here on purpose:** check [40] bans the prefixed forms from every live file, and a note recording what was retired reads exactly the same without them, so this line drops the prefix rather than buying the whole file an exclusion that would waive all 13 of that check's literals to license the 2 really here. `scripts/test_ban_tokens.d/15-wi-absent-cases.sh` writes its own retired type the same way, for the same reason. Every merge below is a union, not a summary: every METHOD step, VERIFICATION item and severity anchor from each retired lens is carried by whatever absorbed it.
