...
This file holds the dispatchable sub-agent prompt for Reviewer D, the performance lens of the Phase 5 multi-reviewer wave. It is the canonical Reviewer D prompt (portable across runtimes); `agents/reviewer-performance.md` mirrors the fenced block byte-for-byte, the copies are identical by design; keep them in sync. The canonical 8-section sub-agent contract lives in `template-contract.md`, do not restate it here. Reviewer A lives in `phase-5-multi-review-a-security.md`, B in `phase-5-multi-review-b-quality-plan.md` and F in `phase-5-multi-review-f-coherence.md`. **The five-agent panel is Phase 5's default reviewer route, in both quick and full mode.** The merged all-lens reviewer (`hackify:reviewer`, `phase-5-multi-review-merged.md`) is the explicit, named, lower-cost opt-out, and a user reaches it by asking for it directly instead of the panel. **When the panel IS the round's reviewer, dispatch all of them in ONE assistant message: A, B, D and F each run on every non-trivial diff, and E joins on a UI-bearing one**.
