...
**Step F. Update log + HTML report** (1 or 2 only). **Runs BEFORE Step D's move**, while the work-doc is still at its live `docs/work/<slug>.md` path and ledger item `6c` is in progress. Print a plain-language **update log**: one block per change the user would recognize, each with five fields in this order, **Problem** / **Root cause** / **Solution** / **Verification evidence** / **Deployment status**, separated by a line containing exactly `----`. Write it the way you would explain the work out loud to someone who was not in the room: everyday words, no jargon they did not use, and never a phase number, task ID, reviewer letter or scout name. The same log is appended to the work-doc under a new top-level `## Update log` heading, and that append rides in Step D's closing edit rather than in an edit of its own. **Then emit a styled, self-contained HTML report** (stats, inline-SVG charts, findings, action items, next steps) straight to its final path, `docs/work/done/<slug>.report.html`, which is where it sits once the move lands, see [references/html-report.md](../html-report.md). **Then, where the runtime can publish a page, publish that report and tell the user the link**, so they end up with something they can send someone rather than a path they have to open by hand. Render the publishable copy in the same run with `--artifact-out "$(mktemp -d)/<slug>.report.body.html"` and hand that file to the runtime's publish tool. A fresh private directory each run, never a fixed name in the shared temp directory, because a predictable path there is one another user can pre-fill with a symlink; the renderer refuses to write through one either way. Where the runtime has no such tool, skip it and say the path: the file is the deliverable and this step is unchanged. **Publishing is never a precondition for closing the ledger.** 6d's exit artifact is the printed log plus the rendered file, so a runtime that cannot publish still finishes the sprint, which is what stops this from becoming the portability bug [references/runtime-adapters.md](../runtime-adapters.md) forbids. Field-by-field guidance, voice rules, and a worked example: `references/finish.md` "Step F (Update log + HTML report)".
