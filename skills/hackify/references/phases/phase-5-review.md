...
**Exit only on a settled diff:** a round that changed any code mandates another round, because that round's clean result describes the pre-fix diff, not the one on disk. **The loop may only end on a FULL round** (the full gated panel, the same gate decision and the same `{{folded_lenses}}` value as round one, and every lens dispatched with a `settle `-prefixed scope) that finds nothing AND leaves every byte of the reviewed diff covered by a live verdict. **The reviewed diff is `git diff <base>..HEAD -- . ':(exclude)docs/work/*'`**, because the work-doc is the ruler the diff is measured against and cannot also be the measured. Writing a round's result into the work-doc changes the work-doc, which kills the work-doc's own verdict, which mandates another round whose result is written into the work-doc again; the loop cannot close. Reviewer B still reads the work-doc in full as an INPUT, so its accuracy stays enforced by every lens that measures the diff against it.
