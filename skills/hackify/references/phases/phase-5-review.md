...
**Yolo is the one mode where the adjudication escalation does not apply.** It ends in a human sign-off, and yolo has nobody to sign a dismissal off, so in yolo a both-lenses-refuted Critical is fixed anyway, both counter-citations recorded beside the fix. No adjudication, no gate, and no verdict takes a Critical out of the address-all loop. Everything else in this rule still binds in yolo: one refuter per round, both lenses, refute before you fix, the default to KEEP, and a Critical refutable only when both of them fail. `skills/yolo/SKILL.md:108` is the authority for that landing.
