...
**Slicing belongs to the panel, and the panel is what a round takes on its own.** The five-agent panel is Phase 5's default reviewer route in both quick and full mode, and the merged all-lens reviewer (`hackify:reviewer`) is the explicit, named, lower-cost opt-out, which takes no `{{review_scope}}` at all because it carries Reviewer B's lens and B is never sliced. So on that named route there is nothing here to resolve, and the one part of this file that still runs is the scope ledger at the bottom, carrying its row per changed path with that single reviewer named as what read it. Everything else below is addressed to the panel, which is most of this file's mechanics on the route a round takes by default. The route, and the measured cost of taking it: [phases/phase-5-review.md](phases/phase-5-review.md).
