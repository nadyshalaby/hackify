...
| **5-lite. Single-lens review** | Dispatch ONE foreground reviewer over the diff carrying every lens at once: quality + engineering law + correctness + goal drift + performance + cross-module coherence. Tell it to **read the touched hunks and the context around them, not whole files**, and to open a file in full only when a finding needs the contract around it, saying why in the finding. Full mode dispatches four or five separate lenses; quick's one reviewer carries all of them on every diff. This reviewer is **composed for quick mode**, not the registered `hackify:code-reviewer-quality-plan` type, so it takes no `{{metrics_table}}`, and counts the size caps (function length, parameters, nesting, file length) itself over the functions the diff touched. Then **refute before you fix** with ONE refuter over every finding in the round, dispatched by type (`hackify:finding-refuter`) with only its INPUTS, `findings_batch` carrying the whole round verbatim and the literal `none` on a round that found nothing rather than a blank, which it refuses, template `skills/hackify/references/parallel-agents/phase-5-refute.md` is for authoring and for runtimes with no agent registry, default is to keep the finding, then the address-all loop: tabulate findings, fix EVERY severity incl. Minor, and **stop there** (one reviewer, one refuter, one fix pass; quick inherits full mode's round cap and does not re-scan). Performance findings cite `perf.<domain>.<slug>` IDs from `rules/performance.md`; law findings cite lawkeeper `rule_id`s. Coherence lens per `skills/hackify/references/parallel-agents/phase-5-multi-review-f-coherence.md`, one implementation agent still leaves seams against existing code. **When the diff is UI-bearing, the same reviewer also carries the design-conformance lens**, audit against `<project>/docs/design/DESIGN.md` if one exists, else the `skills/hackify/references/frontend-design.md` bans. Method: `skills/hackify/references/parallel-agents/phase-5-multi-review-e-design.md`. See `skills/hackify/references/review-and-verify.md` (the single reviewer stands in for the panel, and the round cap applies to it the same way). | Small diffs still ship bugs; one reviewer carrying every lens + refute + address-all is the light-but-real safety net. A one-line CSS tweak is exactly where token drift starts. |
...
| **6F. Update log + HTML report** | Print the plain-language update log per `skills/hackify/references/finish.md` Step F, one block per change with the five fields, then emit the self-contained HTML report per `skills/hackify/references/html-report.md`, and publish it as a link where the runtime can. | The user opted into speed, not opacity. |
...
**Then emit the styled HTML report**, a self-contained `<slug>.report.html` at `docs/work/reports/<YYYY-MM-DD>-<slug>.report.html` (quick has no archived work-doc). Emit a JSON payload and let `skills/hackify/scripts/render-report.py` render it, never hand-write the markup; quick has no base SHA before its first commit, so pass `stats` explicitly. Payload shape: `skills/hackify/references/html-report.md`. Where the runtime can publish a page, publish the report too and tell the user the link, rendering the publishable copy in the same run with `--artifact-out` to a temp path; where it cannot, the path is the deliverable and nothing here changes. Quick has no ledger row to gate, so the rule is the same one full mode states: the file is the artifact, the link is an extra.
...
Clarify-if-ambiguous + goal anchor → implement (one agent, file allowlist) → verify fresh (triad + both scouts + ship gate) → single reviewer carrying every lens, refute, then one address-all fix pass → touched-scope cleanup + plain-language update log + styled HTML report. Stays in quick mode until the user explicitly promotes to full hackify.
