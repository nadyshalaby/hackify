...
| **3. Implement** | **Dispatch. Never write the change yourself**, the no-parent-authored-diff law in `../hackify/SKILL.md` binds here too, down to a one-character typo. Split the ask into file-disjoint units and dispatch one agent per unit in a SINGLE message, grouping units that share a module into one dispatch (the agent takes `{{task_ids}}` and `{{task_descriptions}}`, so a batch of one is just a single-entry list); a genuinely atomic change dispatches alone and you say in chat why it could not be split. Dispatch **by registered agent type** (`hackify:wave-task-implementer`) and pass only its INPUTS, never open the template to paste the prompt, the agent already carries it and reading it charges the same text twice (`../hackify/references/parallel-agents/README.md`). File allowlist applies, each agent touches only its declared files. Quick writes no work-doc, so its `{{repo_brief}}` is the two or three lines you already know, stack, the verbatim test / lint / typecheck commands, and where tests live, stated in chat and passed to the agent so it does not go rediscover them (`../hackify/references/repo-brief.md`). | Every code change goes through the dispatch path, no exceptions for size. Splitting is the first move, not the fallback; promote to full hackify if the task outgrows the carve-out. |
