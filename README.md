...
[![Version](https://img.shields.io/badge/version-0.13.0-7c3aed.svg)](.claude-plugin/plugin.json)
...
- **Three planning reviewers became one, without dropping a single check.** The spec-review step is deliberately non-skippable, so unlike the code-review panel there was no gate that could ever make it cheaper, and all three were reading the same planning document. One reviewer now does all three jobs: does the plan say a consistent thing, can it actually be built in the order it proposes, and would building it break the project's own rules. Every check, every question and every worked example from all three survives, and the report leads with the build order so the next step can read it straight off. Worth knowing: the third fold is a smaller win than it looks, because that reviewer reads the rule files nobody else opens and those reads still happen.
- **The research and debugging helpers stopped being copied out by hand.** Both were written as text the main agent had to read into its own context before it could hand the job off, which meant paying for the same instructions twice. They are proper registered helpers now, and their permissions are narrowed to a read-oriented set so the file-editing tools are simply not on the table.
- **A quiet drift got caught and locked out.** Two helper definitions had been edited by hand and had silently lost pieces of their instructions, including a pointer one reviewer needed. Because the registered definition is the one that actually runs, that was a reviewer running slightly weaker than the written spec promised. Seven definitions are now checked character for character against their source on every build.

### New in 0.12.0

- **Fewer agents doing the same work.** Tasks that touch the same part of the code go to one implementer together instead of one each, so the shared reading happens once. Each task keeps its own file boundary, and a batch stops at the first task it cannot finish, so a bad task costs one task and not the batch. Tasks in unrelated areas still go separately, because there is nothing shared to save.
- **Two checks that stopped running when they had nothing to say.** The cross-module reviewer needs two sides to compare, so on a change that stays inside one area its checklist passes to the quality reviewer instead; two areas touched is enough to run it, and so is one file that anything outside its area uses. And a serious finding is only dropped when two independent checkers both reject it, so the second checker now runs only when the first one votes to drop, which leaves the outcome exactly as it was.
...
- **Phase 2.5. Spec self-review.** Two spec reviewers in parallel (internal consistency plus the execution-wave plan; architectural risk) patch contradictions before any code is written. Non-skippable, small docs are exactly where contradictions hide.
