...
- **The research and debugging helpers became proper registered helpers, and hand-editing them got locked out.** Both were written as text the main agent had to read into its own context before it could hand the job off, which meant paying for the same instructions twice; they are registered now, with permissions narrowed to a read-oriented set so the file-editing tools are simply not on the table. Two definitions had also been edited by hand and had silently lost pieces of their instructions, including a pointer one reviewer needed, and because the registered definition is the one that actually runs, that was a reviewer running slightly weaker than the written spec promised. Seven definitions are now checked character for character against their source on every build.
