...
[![Version](https://img.shields.io/badge/version-0.17.0-7c3aed.svg)](.claude-plugin/plugin.json)
...
For small fixes and single-file edits, a sibling skill `/hackify:quick` runs a compressed flow that stays in quick mode until you explicitly promote to full hackify.
...
│ Phase 3   Implement   foundation wave, module tracks, assembly       │
...
- **Phase 3. Implement.** Three stages, contention first. A solo **foundation wave** lands every contended write in one pass, so nothing else has to queue behind it. Then N **module tracks** run at once, one agent each, every track handing back finished, tested work. A solo **assembly wave** mounts what they built, reconciles it and boots it for real. The shape scales, so none of this is ceremony on a small change: no contended writes means no foundation wave, one track means nothing to assemble, and a stage that does not apply is marked complete with a written reason rather than dropped in silence. Foundation, assembly and any single-track round take the wave implementer; two or more concurrent tracks take the module implementer. A split is permitted only by the three-condition partition test, never chosen by it. Every agent carries a strict file allowlist, stops at the first task it cannot finish, keeps what it landed, and runs both deterministic scouts over its own allowlist before returning; the parent runs them again at round end over what the round declared. Stated in full at `skills/hackify/references/contention-dispatch.md`, summarised here.
