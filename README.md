...
[![Version](https://img.shields.io/badge/version-0.14.0-7c3aed.svg)](.claude-plugin/plugin.json)
...
- **The step tracker can no longer disappear.** On Claude Code the to-do tool it relied on is often simply not there, and nothing was written down about what to do instead, so the checklist quietly vanished and nothing was left holding the steps in order. There is a written fallback now: the checklist is printed in the chat, and on a full run a copy is kept in the work-doc, so it survives closing the session and picking the work up days later. The rules that keep a run in order also arrive with every message instead of being read once at the start and fading as the conversation grows, so the model is still told, on the hundredth message, that steps run in order and one at a time, that a step which genuinely does not apply is marked done with a one-line reason rather than dropped in silence, and that every question or approval comes to you as a proper prompt rather than a numbered list in the chat. Trivial work is exempt: a quick question, a one-line typo fix, or just reading code.
- **A sweep of the instructions caught places where they disagreed with each other.** Two of those disagreements changed what actually happened rather than just how it read. The menu at the end of a task numbered its four choices one way in the overview and another way in the two pages that own it, so the same number meant a different action depending on which page you had read, and the fully automatic mode picks by number rather than by name. A step in the review stage still referred to a helper that had been retired, and the rules these instructions run under make a helper refuse outright when it is handed something that no longer exists, so that step gave nothing back every single time it ran. The rest were quieter: mostly descriptions still promising a fixed set of reviewers after some of them became conditional, plus a few accessibility references pointing at the older version of the standard.

### New in 0.13.0

- **Three planning reviewers became one, and the helper definitions stopped drifting.** The spec-review step is deliberately non-skippable, so unlike the code-review panel there was no gate that could ever make it cheaper, and all three were reading the same planning document; one reviewer now does all three jobs (does the plan say a consistent thing, can it actually be built in the order it proposes, and would building it break the project's own rules), every check, every question and every worked example from all three survives, and the report leads with the build order so the next step can read it straight off. Worth knowing: the third fold is a smaller win than it looks, because that reviewer reads the rule files nobody else opens and those reads still happen. The research and debugging helpers became proper registered helpers too, with permissions narrowed to a read-oriented set so the file-editing tools are simply not on the table; both had been written as text the main agent had to read into its own context before it could hand the job off, which meant paying for the same instructions twice. Two definitions had also been edited by hand and had silently lost pieces of their instructions, including a pointer one reviewer needed, and because the registered definition is the one that actually runs, that was a reviewer running slightly weaker than the written spec promised. Seven definitions are now checked character for character against their source on every build.

### New in 0.12.0

- **Less work for the same coverage, in two places.** Tasks that touch the same part of the code go to one implementer together instead of one each, so the shared reading happens once; each task keeps its own file boundary, a batch stops at the first task it cannot finish so a bad task costs one task and not the batch, and tasks in unrelated areas still go separately because there is nothing shared to save. Two checks also stopped running when they had nothing to say: the cross-module reviewer needs two sides to compare, so on a change that stays inside one area its checklist passes to the quality reviewer instead (two areas touched is enough to run it, and so is one file that anything outside its area uses), and a serious finding is only dropped when two independent checkers both reject it, so the second checker now runs only when the first one votes to drop, which leaves the outcome exactly as it was.
