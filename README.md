...
[![Version](https://img.shields.io/badge/version-0.15.0-7c3aed.svg)](.claude-plugin/plugin.json)
...
- **A round of work now goes to one implementer instead of several, and the cost of that is written down rather than glossed over.** Work is planned in rounds, and a round used to be split across several implementers running side by side, each handed a slice and each having to read the same background before it could start. One implementer now takes the whole round, in order. The repeated reading happens once instead of once per slice, which is a large saving in tokens, and everything in a round is decided in one place, so the two halves of a change fit together instead of being stitched from separate accounts of the same thing. It is slower on the clock, and that is a genuine cost rather than a rounding error: several implementers working at once finish sooner than one working through the same list in order. The trade was made with that said out loud. What pays for it is the rule that an implementer which runs into something it cannot finish stops there, keeps everything it has already done, and says exactly how far it got, so a bad item costs one item and not the whole round. That was put to the test twice while building this release and nothing had to be redone, though one of the two interruptions did leave a job half-finished until the next round picked it up. The step that argues back against review findings collapsed the same way, one of them for a whole round instead of several, and a serious finding is still only dropped when it fails on both counts, whether it can be reproduced and whether the rule it cites is real, so the bar stayed exactly where it was even though one checker now does the work that used to take two.
...
│ Phase 3   Implement   one foreground subagent per whole wave         │
