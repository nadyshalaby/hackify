...
### W3 — Delete originals + Phase 6 spec extension + v0.2.7 — done 2026-05-21

3 parallel agents (T12, T13, T14) + 1 fix-up (T18 — `validate-dod.d/10-required-files.sh` recursive count).

- **T12** (delete 2 originals + fix W2/T7's stale ref): repointed `agents/spec-reviewer-dependencies.md` line 87 to `../skills/hackify/references/parallel-agents/README.md` before deletion. `git rm`'d both originals. Agent **discovered the 10-required-files.sh blocker** and STOPPED per protocol (file outside its allowlist). Surfaced for fix-up.
- **T13** (Phase 6 Step C.5): inserted into `skills/hackify/SKILL.md` L246 (between Step C and Step D) as an 8-row summary table; full per-class detail authored in `skills/hackify/references/finish.md` L117+ as a `## Step C.5 — Cleanup sweep` section. `skills/yolo/SKILL.md` got the 1-line "applies here too" pointer; quick/SKILL.md skipped (quick explicitly skips Steps C and D — Step C.5 lives between them, not applicable). Agent also **discovered the same 10-required-files.sh blocker** independently.
- **T14** (v0.2.7 bump): `plugin.json` + `marketplace.json` both at 0.2.7 (lockstep); CHANGELOG `## [0.2.7] - 2026-05-21` entry with 6 bullets covering all 7 required topics; README version badge updated. Agent **also discovered the same 10-required-files.sh blocker** — third independent confirmation.
- **T18** (validate-dod.d/10-required-files.sh fix-up): added during W3 to unblock DoD. `find -maxdepth 1` → `find` (recursive); threshold `≥10` → `≥20` (actual 31 with the new subdir layout). One header comment line documents the v0.2.7 rationale.

- **Wave verification.** Originals confirmed gone. Step C.5 grep returns 1 hit per file (SKILL.md L246; finish.md L117). Both JSONs at `0.2.7` (lockstep). README badge at `version-0.2.7`. CHANGELOG `## [0.2.7]` heading present. Recursive references-tree count = 31 files (well over the new ≥20 threshold). **`bash scripts/validate-dod.sh` → `ALL CHECKS PASSED`** (the blocker is fixed).

- **Self-review.** ✓ DRY (Step C.5 summary in SKILL.md / detail in finish.md; not restated) ✓ no suppressions ✓ scope creep contained (T18 fix-up was a direct corollary of T12's deletion; not new feature) ✓ originals deleted cleanly (no forwarding stubs) ✓ version lockstep verified across 4 surfaces (plugin.json, marketplace.json, CHANGELOG, README badge).
