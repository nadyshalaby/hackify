...
- **Landmines.** (a) **Agent mirrors copy only the fenced block; frontmatter is NOT mirrored**, which has already caused two defects. (b) `70-invariants-and-new.sh` pins exact literal strings; rewording a pinned line breaks the pin silently if the pin is a prefix match. (c) README caps at 450 and sits at **449**, one line of headroom. (d) The three fragments that could not gain a line were split in wave 22 of the ledger sprint; the moved checks live in `71-release-mechanism-pins.sh` and `79-standing-member-invariant.sh`. **Counted against the 500 cap on this diff, not quoted from that sprint:** `70-invariants-and-new.sh` **298** (was 145 there, and this sprint's pins are what grew it), `71-release-mechanism-pins.sh` **496**, four lines of headroom, so a finding that lands there needs a split plan and not an edit; `79-standing-member-invariant.sh` 281; `77-reviewer-roster.sh` 269. (e) `block-banned-tokens.sh` rejects em dashes. (f) `dist/copilot-cli/` is MANIFEST-only by design.
