...
> **Minor-level: edit-time ban enforcement + a CI gate + DoD/manifest hardening.** A new `PreToolUse` hook blocks banned tokens as they are written (not just caught in review); GitHub Actions now runs every gate on push/PR; and two new Definition-of-Done checks close the holes that let a stale README badge and 5 unmirrored evals ship. The plugin-hardening pass — areas surfaced by auditing hackify against its own doctrine.

### Added

- **`hooks/block-banned-tokens.sh` + `hooks/scan_edit.py`** — a `PreToolUse` (Write|Edit) hook that blocks edits introducing zero-tolerance banned tokens into JS/TS source: lint/type suppressions (`@ts-ignore`, `@ts-nocheck`, `eslint-disable`, `biome-ignore`; `@ts-expect-error` outside test files), non-null `!`, empty `catch {}`, and bare `throw new Error(`. Detection reuses lawkeeper's tested `lexer.py` + `checks.py` regexes (single source of truth) — semantic bans are matched on lexer-MASKED text, so a token inside a string or comment never false-fires; suppressions are matched on raw text. **On by default** (claude-code only). Per-path escape hatch: list a path (literal or glob) in `<project-root>/.claude/hooks/ban-allowlist` (e.g. standalone front-end assets where a bare `Error` is acceptable). Fail-open by design: any internal failure (missing `jq`/`python3`, unparseable input) allows the edit — a hook bug must never wedge editing. 16-case test suite (`hooks/test_block_banned_tokens.sh`).
...
- **`README.md`** — new **Skill routing** matrix (intent → skill, with the "audit / review / check" overlap disambiguated across all 8 skills); the hooks-primitive and repository-layout sections now document the `PreToolUse` ban-blocker; version badge → 0.5.0.
