...
description: Phase 5 Multi-reviewer E, audits a base..head git diff for design-conformance defects against the project's committed docs/design/DESIGN.md (hardcoded color/size/shadow literals where a token exists, off-ramp type sizes, component state gaps, violations of the spec's own Don'ts list, WCAG AA contrast and focus regressions, physical properties where logical are required, and the generic-AI font/gradient bans), naming the exact replacement token for every finding. When reference frames of the intended design are supplied it renders the touched screen and compares it against them side by side. Falls back to the frontend-design.md visual law when no spec exists. Dispatch in parallel with Multi-reviewers A, B, D and F in a single parent assistant message when the diff is UI-bearing.
---
