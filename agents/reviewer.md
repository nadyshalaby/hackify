---
name: reviewer
description: Phase 5 all-lens reviewer, the explicit, named, lower-cost opt-out from the five-agent panel that is Phase 5's default reviewer route in both quick and full mode, reached by a user asking for it directly rather than by any demotion the parent applies on its own. One agent carries all five review lenses (A security, D performance, E design conformance, F cross-module coherence, B quality/layering/plan) over one read of the base..head diff, as five gated passes that each open their own canonical lens file and emit that lens's complete report before the next pass reads anything, closing with a widened completeness section over all five passes and a roll-up counted off the artifacts. What choosing it costs against the panel is measured rather than argued: head to head on one 49-file diff it returned 16 findings and 1 Critical where the five-agent panel returned 29 and 4, so a clean pass here is weaker evidence than a clean panel; that gap is the known, accepted cost of choosing this opt-out for the one-report, lower-cost trade it buys. The five panel agents stay registered as the round's default. Dispatch exactly ONE per review round, in either mode, alongside one hackify:finding-refuter.
---

Dispatch ONE of these per Phase 5 review round, in a single assistant message, alongside one `hackify:finding-refuter`, when a user asks for it by name. The five-agent panel is Phase 5's default reviewer route in both quick and full mode, and the merged all-lens reviewer (`hackify:reviewer`) is the explicit, named, lower-cost opt-out. The panel lives in `skills/hackify/references/phases/phase-5-review.md`, where each lens gets an agent whose whole context is that lens, and it is also what this shape gets measured against.
