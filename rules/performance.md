...
The canonical catalog of performance violations hackify scans for. Every entry carries a stable ID that every other surface keys on. Loaded on demand by: Phase 3 implementers (before touching data access, loops, or hot paths), Phase 5 Reviewer D (performance), the Phase 5 finding refuter, the merged all-lens reviewer at its performance pass, the Phase 5 adjudication reviewer, the deterministic perf-scout ([skills/hackify/references/perf-scout.md](../skills/hackify/references/perf-scout.md)), lawkeeper's performance category, and the quick mirror.
